...
import pandas as pd
import ee
from shapely import wkt as shapely_wkt  # Added for WKT parsing
...
def create_ee_geometry(kwargs: Params):
    ...
    # Polygon WKT support takes precedence over area
    if getattr(kwargs, 'polygon', None):
        logger.info(f"Using polygon geometry from WKT: {kwargs.polygon}")
        shapely_geom = shapely_wkt.loads(kwargs.polygon)
        exterior_coords = list(shapely_geom.exterior.coords)
        return ee.Geometry.Polygon(exterior_coords)
    # old area logic (disabled)
    # elif kwargs.area is not None:
